...
> `lurkinghorror.txt`) is now fully labeled — **52 Done** (mostly already-built; the
> "we don't have" framing was stale), **18 Won't do**, **18 Will do**. Shipped 2026-07-11:
...
> goldens `switchdevice1`/`switchdevicefr1`); and `listen`/`smell` distance senses (`sound`/
> `scent`, en-US + fr-FR, goldens `sense1`/`sensefr1`). Remaining Will-do work
> clusters into: enter/exit + vehicles (incl. non-vehicle enterables); `take X from Y`
> scope-narrowing; prepositional
