...
4. `lib/advent` migration — **planned 2026-06-25**, sub-sequenced so each commit stays green and stdout is invariant (no pinned generated-JS to regen). **Key discovery:** raw `.holder` *field* reads (advent AND cloak.lamp's ~12 own-rule reads, scattered fixtures) go stale the moment writes switch to `contains`, so a naive flip is a huge atomic change. **Bridge (decouples advent from fixtures):** (a) step-3 `containerOf` holder fallback covers field-placed/never-moved items; (b) a transitional advent handler `on contains add: self.contained.holder = self.place` keeps the legacy field correct for anything still reading it. No sample/fixture registers its own holder-change handler (checked), so the bridge can't collide.
   ...
   - **4b (fixtures/samples → contains placement, batched, deferrable):** `holder X` field placements → `contains X obj`; cloak.lamp own-rule `.holder` reads → `holder()`. Green per batch (bridge active).
...
- **Parser: allow property access on a call/parenthesized result — e.g. `holder(p).lighted`, `(EXPR).field`.** Today postfix `.field` only chains off a variable or object reference; `holder(p).lighted` is a compile error ("property access requires a variable or object reference, not a literal value"), forcing a `let here = holder(p)` workaround (used in advent `in_darkness` and would tidy other call-then-access sites). Generalize the primary/postfix parse so `.field` (and presumably `?`-query slots? — no, slots stay atoms) chains off any primary expression incl. a function call and a parenthesized expression. Mind the checker's `resolveChainType`, which assumes a chain rooted at a name. **Requested 2026-06-25.** **Where:** `src/lantern/parser_rd.js` (primary/postfix), `src/lantern/checker.js` (chain typing).
