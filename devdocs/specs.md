...
- `lib/sys/` is the system library. Every invocation of Lantern automatically parses all `.lamp` files in `lib/sys/` — no explicit import is required. The following built-in functions are available to all Lamp programs (native implementations in the sys library's `index.js`, except the entries marked pure Lamp, which live in `functions.lamp`):
...
- `message(name, default)`
    - Returns the registered override text for the named message, else `default`. The emit target of the inline message form `NAME:"DEFAULT"` (a localizable message whose default lives at the use site). The default-less reference form `message NAME` emits `message(name)` with no fallback — the checker requires a `NAME: "…"` registration in some loaded file (normally the library's locale file, `lib/<lib>/locales/<tag>.lamp`), so a missing translation is a compile error; at runtime an unregistered name renders `[missing message: NAME]`. See `devdocs/messages.md`.
...
- A **`sugar` declaration** — `sugar bare WORD[, …]` / `sugar operand WORD[, …]` at top
  ...
  token is zero-arg (`[we]`); an `operand` token takes one reference (`[the X]`). Each WORD
  maps to a native call — the coerced word by default, or `"quoted" as native` when the
  token isn't a valid native name (an apostrophe: `"we're" as we_re`) or differs from the
  function (`a as indefinite`). Like `verb`, it is inert (the prescan collects the map that
  drives desugaring). **The compiler bakes in no language sugar** — a locale (or a game)
  declares its own, so a new language needs no compiler change. `lib/en-US`/`lib/fr-FR`
  ship the built-in sets. See devdocs/i18n.md ("declarable grammar sugar").
...
| `set_verbose` / `set_brief` / `set_superbrief` | — | `verbose`/`long`, `brief`/`normal`, `superbrief`/`short` | The Infocom display modes (out-of-world, no turn). Set the `verbosity` global (enum `verbose`/`brief`/`superbrief`, default `verbose`), which controls the auto-description on **arrival**: VERBOSE always full; BRIEF full on a room's first visit and name-only after (via the room's `visited` flag); SUPERBRIEF name-only always. An explicit `look` is always full. Confirm with `verbose_report` / `brief_report` / `superbrief_report`. See `describe_on_arrival` (rooms.lamp). |
| `examine` | `direct physical target` | `examine [target]`, `x [target]`, `read [target]` | Prints `self.target.description` (or `examine_nothing` when empty). Targets any `physical`, so people and non-item scenery are examinable too. When the target is an **open container** (`is_container` and not closed) holding something, appends Inform's contents listing — `examine_in_container` "In [the target] is …" / `examine_in_container_plural` "… are …" (agreeing in number). Gated to container kinds, so examining a person doesn't spill their inventory (golden `examinecontents1`). Examining **yourself** (`x me`/`myself`/`self`, the self-words resolving to the acting actor) with no description prints `examine_self` ("[We] [are] as good-looking as ever.") instead of the generic line; a game overrides via the actor's `description`. Golden `examineme1`. |
| `look_direction` | `direction way` | `look [way]` (e.g. `look down`, `look north`) | Glances a compass/vertical direction — distinct from bare `look` (describe the room) by the direction slot. Succeeds with `look_direction_nothing` = "[We] [see] nothing unexpected that way."; a game overrides per direction (`instead look_direction when self.way == down: …`). No reach/light gating. |
| `look_under` / `look_behind` | `direct physical target` | `look under [target]`, `look behind [target]` | **Fail by default** (shared reason `nothing_there`) like `push`: most things hide nothing, so the default is `look_under_nothing` = "[We] [find] nothing of interest there." / `look_behind_nothing` = "There is nothing behind [the target]." A game adds `instead look_under` / `instead look_behind` (e.g. to reveal a key under a rug). |
| `look_in` | `direct physical target` | `look in [target]`, `look inside [target]`, `look through [target]` | Lists an **open container's** contents (`look through` is a synonym). Refuses a non-container (`not_container`, `look_in_not_container`); reports a closed one (`look_in_closed`) or an empty one (`look_in_empty`); otherwise lists with number agreement (`look_in_contents` / `_plural`, like EXAMINE's container branch, via the shared `render_container` helper). Reach/light-gated like EXAMINE (`too_dark`). |
| `search` | `direct physical target` | `search [target]` | Succeeds for a **container** (lists its contents via `render_container`, shared with `look_in`) or a **supporter** ("On [the target] is …" via `supporter_phrase`; empty → `search_nothing_on`). **Fails by default** for anything else (`nothing_found`, `search_nothing` = "[We] [find] nothing of interest.") so a game adds `instead search X` for scenery that hides things. Reach/light-gated like EXAMINE. |
| `touch` | `direct physical target` | `touch [target]`, `feel [target]` | Inform's "touching" (ported from Can't Touch This.i7x). Senses a thing without changing the world (no `do` band, like `examine`), and works in the dark. Prints the target's `feels` text when set, else `touch_nothing` = "[We] [feel] nothing unexpected." Refused when the target is `unfeelable` (reason `cant_touch`, message `touch_cant` = "[We] can't touch [regarding act.target][those].") or out of reach — `far_away`/`obstructed` (reason `cant_reach`, message `touch_cant_reach` = "[We] can't reach [regarding act.target][those]."). The reach/feel traits also refuse `take` (see below). Targets any `physical`. `target` is `direct`, so `it` refers back to it. |
| `listen` / `listen_to` | (none) / `direct physical target` | `listen`, `listen to [target]` | Distance sense (Inform's "listening to"). Senses without changing the world (no `do` band) and works in the dark; **no reach gating** (unlike `touch`/`taste`). Objectless `listen` prints the actor's room `sound`; `listen to X` prints the target's `sound`; empty → `listen_nothing` = "[We] [hear] nothing unexpected." A non-empty string is printed verbatim (game content). Ordinary in-world action (spends the turn), so a game's `instead listen` can vary the sound over turns. |
| `smell` / `smell_thing` | (none) / `direct physical target` | `smell`, `sniff`, `smell [target]`, `sniff [target]` | Distance sense (Inform's "smelling"), mirroring `listen`. Objectless `smell` prints the room `scent`; `smell X` prints the target's `scent`; empty → `smell_nothing` = "[We] [smell] nothing unexpected." No reach gating; works in the dark; spends the turn. |
...
| `sleep` | — | `sleep`, `nap` | Objectless. **Fails by default** (`cant_sleep`, `sleep_refused` = "[We] [are] not feeling especially drowsy.") so a game overrides with `instead sleep` to narrate rest / a dream. Ordinary in-world action, so a refused attempt still spends the turn and every-turn rules fire (a scripted dream can run on `every_turn_rules`). French `dormir`. |
| `switch_on` | `direct item target` | `turn on [target]`, `turn [target] on`, `switch on [target]`, `switch [target] on` | Inform's "switching on". Does **only** one thing — sets `switched_on` true on a `switchable` thing (`switch_on_report` = "[We] [switch] on [the target]."). Refuses a non-`switchable` thing (`not_switchable`, `switch_on_not_switchable`) and an already-on one (`already_on`, `switch_on_already`). Owns no device behaviour: a game wires consequences (a flashlight's `report switch_on` links `switched_on` → `lit`; a forklift reports "sputters to life"). `switchable`/`switched_on` live on `physical`, so a machine may also be `scenery`. `target` is `direct`, so `it`/`them` refers back. |
| `switch_off` | `direct item target` | `turn off [target]`, `turn [target] off`, `switch off [target]`, `switch [target] off` | Inform's "switching off" — the counterpart to `switch_on`. Sets `switched_on` false (`switch_off_report`); refuses a non-`switchable` thing (`not_switchable`) and an already-off one (`already_off`, `switch_off_already`). |
...
| `take_from` | `item taken` (multi), `physical source` | `take [taken] from [source]`, `get … from …`, `take … out of …`, `remove … from …` | Ordinary TAKE with the FROM clause **narrowing the noun scope**: `taken` resolves within `source`'s contents (registered via `setSlotScopedByContents`, so the runtime scopes the slot — not a new world verb). Disambiguates a same-named object (`take coin from crate` → the crate's, not the floor's) and bounds `all` (`take all from coffer` → only its contents). A closed/empty source or an object not inside it yields **no** match → the generic "can't see any such thing" (no bespoke message, matching Inform). `do` delegates to TAKE per object, so all its checks and reports apply. French `prendre … dans/sur …`. |
| `drop` | `item dropped` | `drop [dropped]` | Moves item to actor's location; a worn item is implicitly taken off first via **`implicitly_doff`** (the shared implicit-action facility — announces `(first taking off X)` for the player, tries doff silently, bails on failure). |
| `inventory` | — | `inventory`, `i` | Lists carried items; marks worn items with `(worn)` and `lit` items with `(providing light)`. |
| `wear` | `item clothing` | `wear [clothing]` | Asserts `wears actor clothing`; an uncarried item is implicitly taken first via **`implicitly_take`** (the shared implicit-action facility — announces `(first taking X)`, tries take silently, bails on failure; `silently` suppresses only the SUCCESS report, so "(first taking the cloak)" leads straight to "You put on the cloak." while a failed take still prints its refusal). A game action uses the same helpers (`if not implicitly_take(self.actor, self.target): stop failed` — golden `implicit1`); the announcements are the shared `implicit_take_msg`/`implicit_doff_msg`, naming the object via the `implicit_target` global. |
...
| `enter` | `direct physical target` | `enter [target]`, `get in/into/on/onto [target]`, `sit on/in [target]` | Get on/in an **`enterable`** thing (chair, bed, closet) — the non-vehicle model, following Inform: the actor is **moved into/onto** the thing (holder relocation) and can **nest** (get in a closet, then an undercloset). `scopeOf` reaches out through open enclosures so the room stays visible/reachable; the LOOK heading gains the `(in the closet) (in the undercloset)` chain and the description gains layered "In the X you see …" paragraphs (`describe_enclosures_from`, `enclosure_clause` / `enclosure_contents_phrase` locale natives). After entering, the thing's contents are described. "on" for a `supporter`, "in" otherwise. Refuses a non-`enterable` (`cant_enter`; the **`vehicle`** flag implies enterable), a **closed** one (`enter_closed` — open it first), or the one already occupied (`already_entered`). `location_of`/`in_darkness` walk up to the enclosing room. |
| `exit_object` | — | `get off`, `get out`, `get up`, `stand`, `stand up` | Move to the holder of the thing the actor is on/in (an outer enterable, or the room); `last_exited` names it in the report (`exit_off_report` / `exit_out_report`), and `arrival_silent` keeps an exit-to-room from re-describing. Refuses when directly in the room (`not_perched`) or when the enclosure is **closed** (`exit_closed`) — the trap mechanic: the enclosure stays referable from inside (`scopeOf` includes the actor's holder chain), so OPEN resolves from within, but a locked/game-sealed container genuinely holds the player (the library never guarantees escape). Distinct from the `out`/`exit` **direction** words (they still drive `go`); a `go` while nested reports `go_perched` ("[We] [need] to get out of [the holder] first."). French `se lever` / `debout`. |
...
| `ask_for` | `visible physical interlocutor`, `direct visible item requested` | `ask [interlocutor] for [requested]` | The reverse of GIVE (devdocs/conversation.md): **fails by default** (`ask_for_declined`) — a game's `instead ask_for` hands the thing over. `requested` is `visible` (you ask; the NPC does the transfer) and `direct` (the natural "it"). French `demander [requested] à [interlocutor]`. |
| `go` | `direction way` | `go [way]`, `[way]` | Moves actor along a `connects` edge — or **drives**: when the actor sits directly in/on a **`vehicle`** that itself stands in a room, the direction moves the *vehicle* (the rider rides along via containment) and the arrival re-describes with the "(on the forklift)" heading (`describe_player_arrival`, since no containment event fires for the rider). Any other enclosure (a chair, a nested seat, a vehicle parked inside something) refuses with `go_perched`. Fit constraints ("the forklift won't fit into the stairwell") are per-game `check go` rules (author order, run before the library's). Golden `vehicle1`. Directions: the eight compass points, `up`/`down`, and `inward`/`outward` (typed `in`/`out`, named that way because `in` is a reserved keyword). **Route-finding**: `route_to(here, destination)` (advent, pure Lamp) returns the first step of a shortest route over the `connects` graph as a direction — or `none` when already there or unreachable. Breadth-first (shortest), ties broken by direction declaration order (deterministic); doors are not considered — passage is adjudicated by the go checks when the step is tried, so a closed door surfaces as a failed `try`, not a missing route. The NPC driving idiom: `try go: actor npc, way route_to(location_of(npc), goal)` — one step per turn, narrated by the witnessed-movement report. Golden `routeto1`. **NPC movement is witnessed, not narrated** (devdocs/phobos_gaps.md §9): `try go: actor npc, way south` runs this same pipeline — the checks bind the NPC (closed/locked doors refuse it), and the report prints what the *player* sees: a departure line when they share the origin (`npc_go_departs`, "[The act.actor] goes [act.way]."), an arrival line at the destination (`npc_go_arrives`, "… arrives from [the act.way.inverse]."), nothing elsewhere. `go` carries a never-grammar-bound `room origin` slot, stashed by `do` before the move, for exactly this. A **failed** NPC go is silent — the failure reports are player-only (unmarked) — and the driver reads the `try` outcome instead (`let r = try go: …`). Golden `npcgo1`. A blocked exit (`cant_go_that_way`) gives a **direction-aware** excuse (Can't Go That Way.i7x): `up` → `go_cant_up` ("[We] can neither climb walls nor fly."), `down` → `go_cant_down` ("[We] can't just dig downward."), `inward` → `go_cant_in` ("What [do] [we] want to enter?"), else `go_cant` ("[We] can't go that way."). A game refines per-room by contributing a `report failed go` rule **from its main file** (author order, runs first). |
