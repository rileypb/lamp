...
| `wear` | `item clothing` | `wear [clothing]` | Asserts `wears actor clothing`; an uncarried item is implicitly taken first via **`implicitly_take`** (the shared implicit-action facility — announces `(first taking X)`, tries take LOUDLY so its "Taken."/refusal composes, bails on failure). A game action uses the same helpers (`if not implicitly_take(self.actor, self.target): stop failed` — golden `implicit1`); the announcements are the shared `implicit_take_msg`/`implicit_doff_msg`, naming the object via the `implicit_target` global. |
