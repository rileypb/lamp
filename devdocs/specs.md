...
**Optional string encoding (`--encode-strings`).** When this flag is passed, the emitter wraps player-facing string *values* as `lamplighter.decode("…")` over an encoded payload instead of plain JS string literals. Only prose/value literals (those emitted via `emitValue`/`emitExpression` and type-level field defaults) are encoded; structural strings — object/type/relation/action names, enum labels, field keys, and object references via `getObject` — stay plaintext so runtime lookups are unaffected. The shared codec (`src/strcodec.js`, XOR + base64) is reversible by design: its purpose is only to make spoilers inconvenient for a casual reader of a shipped bundle, **not** to provide security — the decoder and key ship together. The flag is off by default (readable output for development); Lighthouse turns it on for distribution builds. Decoding runs when each literal is evaluated, so behavior is identical to the plaintext build.
