...
**Optional string encoding (`--encode-strings`).** When this flag is passed, the emitter wraps player-facing strings as `lamplighter.decode("…")` over an encoded payload instead of plain JS string literals. Encoded: prose/value literals (via `emitValue`/`emitExpression` and type-level field defaults); object names and global names at *every* emission site (`createObject`/`getObject`/`objectRef`/`checkedGetObject`, `defineGlobal`/`setGlobal`/`getGlobal`), routed through the shared `emitName` helper; and **grammar templates** (`registerGrammar`) and **relation syntax templates** (`defineRelation`), the player-visible command phrasing — both parsed at runtime from the decoded string. (The action *name* in `registerGrammar`'s first argument stays plaintext — see below.) This is behavior-preserving because `decode` runs at load: object/global names are registry keys, but the runtime key is the decoded plaintext, identical to the plaintext build, so lookups and `===` identity are unchanged. Left plaintext: type/relation/action names, enum labels, and field/property keys (these are language structure, rarely spoilers, and field keys would need dynamic property access); the JS `const <objectName>` binding stays a plain identifier (minify mangles it). Note that an action *name* (e.g. `"hang"`) therefore still appears via `registerGrammar`/`registerActionRule`/`runAction`, so the bare verb leaks even though its full grammar phrasing is hidden; encoding action names would require routing every action-name site (including their use as type/dispatch keys) through `emitName`. The shared codec (`src/strcodec.js`, XOR + base64) is reversible by design: its purpose is only to make spoilers inconvenient for a casual reader of a shipped bundle, **not** to provide security — the decoder and key ship together. Strings inside inlined native `index.js` are not touched (the emitter does not parse native JS). The flag is off by default (readable output for development); Lighthouse turns it on for distribution builds.
