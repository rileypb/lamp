...
| `enter` | `direct physical target` | `enter [target]`, `get in/into/on/onto [target]`, `sit on/in [target]` | Get on/in an **`enterable`** thing (chair, bed, closet) — the non-vehicle model. Records a `perch` on the actor (`person.perch`); the actor stays in the room, so the perch just annotates the heading (", on the chair" / ", in the closet" via `perch_phrase`) and blocks `go` until they get off. "on" for a `supporter`, "in" otherwise. Refuses a non-`enterable` (`cant_enter`) or the one already occupied (`already_entered`); a game still adds `instead enter` for a ride-able vehicle (a separate, later model). `target` is `direct`. |
| `exit_object` | — | `get off`, `get out`, `get up`, `stand`, `stand up` | Leave the enterable object the actor is on/in (clears `perch`): `exit_off_report` "[We] [get] off …" for a supporter, `exit_out_report` "[We] [get] out of …" otherwise. Refuses when not perched (`not_perched`). Distinct from the `out`/`exit` **direction** words, which still drive `go`; while perched, a `go` reports `go_perched` ("[We] [need] to leave [the perch] first."). French `se lever` / `debout`. |
