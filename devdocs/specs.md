...
| `enter` | `direct physical target` | `enter [target]`, `get in/into/on/onto [target]`, `sit on/in [target]` | Get on/in an **`enterable`** thing (chair, bed, closet) — the non-vehicle model, following Inform: the actor is **moved into/onto** the thing (holder relocation) and can **nest** (get in a closet, then an undercloset). `scopeOf` reaches out through open enclosures so the room stays visible/reachable; the LOOK heading gains the `(in the closet) (in the undercloset)` chain and the description gains layered "In the X you see …" paragraphs (`describe_enclosures_from`, `enclosure_clause` / `enclosure_contents_phrase` locale natives). After entering, the thing's contents are described. "on" for a `supporter`, "in" otherwise. Refuses a non-`enterable` (`cant_enter`), a **closed** one (`enter_closed` — open it first), or the one already occupied (`already_entered`); a game adds `instead enter` for a ride-able vehicle (later model). `location_of`/`in_darkness` walk up to the enclosing room. |
