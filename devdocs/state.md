...
> was identical). Phases 1–2a of `devdocs/text-persistence.md` fix this for every
> template that reads globals/functions/literals **or a named instance** (`[clock.hour]`),
> whether set in a **construction default or assigned in a rule** — the emitter tracks
> lexical scope so a named instance persists as a module const unless a local shadows it.
> Regressions: goldens `textlive1` (construction, global + named-instance) and `textlive2`
> (rule-assigned, named-instance), both undo *and* save/restore. As a bonus, a persisted template is no longer *rendered* at capture, so it
