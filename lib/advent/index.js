// A closed OPAQUE container seals its contents out of scope (mirrors contents_of hiding them
// from listings): you can't take or examine what's inside a shut box until it opens. A
// `transparent` box (a glass case) does NOT seal — its contents stay visible/referable and its
// light shines out — but reach is still blocked (reach_barrier, rooms.lamp). Restricted to actual
// `container` types (a box) — a `door` also has a `closed` field, but it's a passage, not a
// vessel, so a closed door must NOT hide its parts (e.g. a scanner).
lamplighter.registerScopeBarrier(
    (container) => lamplighter.isTypeOrSubtype(container.type, "container")
        && !!container.closed && !container.transparent,
);
...
// Register FROM-scoped verbs: `take X from Y` resolves its `taken` slot within `source`'s
// contents (see actions.lamp `take_from`). Called once at startup, after action types exist.
function wire_verb_scoping() {
    lamplighter.setSlotScopedByContents("take_from", "taken", "source");
}
...
lamplighter.registerScopeProvider(function (actor, location) {
    // A region-scoped backdrop is present only in its region's rooms (nesting
    // included); an unscoped one is present everywhere, as before. Climb the
    // location to its room first (the actor may be inside an enterable).
    let room = location;
    while (room && !lamplighter.isType(room, "room")) room = lamplighter.containerOf(room);
    return lamplighter.listItems(lamplighter.type("backdrop").all).filter((b) => {
        if (!b.region) return true;
        let g = room ? room.region : null;
        while (g) {
            if (g === b.region) return true;
            g = g.region;
        }
        return false;
    });
...
// A room offers light to an actor when any object in the actor's scope is `lit` (a carried
// flashlight, a glowing thing on the floor). Reuses scopeOf so the closed-container barrier
// applies: a lit thing sealed in a shut box casts no light. The room's own `lighted` flag is
// checked separately in Lamp (in_darkness); this answers only the carried/present-light half.
function light_in_scope(actor) {
    return lamplighter.scopeOf(actor).some((x) => !!x.lit);
}
