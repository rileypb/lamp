...
// Whether `x` is a container kind (a box, or any `container` subtype) — as opposed to a person or
// other physical that merely holds things. Gates the examine-time contents listing so examining a
// person doesn't spill their inventory. Mirrors the scope barrier's container test.
function is_container(x) {
    return !!(x && lamplighter.isTypeOrSubtype(x.type, "container"));
}
