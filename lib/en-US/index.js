...
// choices, set by the globals `viewpoint_person` (1/2/3, default 2 — "you") and
// `viewpoint_plural` (default false). Gender (which only surfaces in third person) is read
// off the *player object* — its `gender` field, the same source the subject pronouns use —
// so it follows the main character rather than a separate global that could desync. Absent
// a player or an unrecognized gender, it falls back to neuter ("it"). Globals are saved.
function viewpoint() {
    ...
    return {
        ...
        gender: gender_of(lamplighter.getGlobal("player")),
    };
}
