...
function we_re()    { return viewpoint_contraction((d) => AUX_CLITIC[aux_form("be", d)], (d) => aux_form("be", d)); }
function we_ve()    { return viewpoint_contraction((d) => AUX_CLITIC[aux_form("have", d)], (d) => aux_form("have", d)); }
function we_ll()    { return viewpoint_contraction(() => "'ll", () => "will"); }
...
function they_re()    { return referent_contraction((d) => AUX_CLITIC[aux_form("be", d)]); }
function they_ve()    { return referent_contraction((d) => AUX_CLITIC[aux_form("have", d)]); }
function they_ll()    { return referent_contraction(() => "'ll"); }
...
function dont()   { return aux_form("do", contraction_agreement()) === "does" ? "doesn't" : "don't"; }
function arent()  { return aux_form("be", contraction_agreement()) === "is" ? "isn't" : "aren't"; }
function werent() { return be_past(contraction_agreement()) === "was" ? "wasn't" : "weren't"; }
function havent() { return aux_form("have", contraction_agreement()) === "has" ? "hasn't" : "haven't"; }

// D9c demonstrative contraction (pairs with [those]): "that's" singular, "those are" plural
// (English has no "those're"). Reads the agreement like [those].
function thats() { return contraction_agreement().plural ? "those are" : "that's"; }
...
function supporter_phrase(supporter, contents, count) {
    const verb = Number(count) === 1 ? "is" : "are";
    return "On " + the(supporter) + " " + verb + " " + contents + ".";
}
