...
// " (on the chair)" / " (in the closet)" — one heading clause per enclosing enterable.
function enclosure_clause(p) {
    if (!p) return "";
    return (p.supporter ? " (on " : " (in ") + the(p) + ")";
}

// "In the closet you see a key." / "On the chair you see a cushion." — a layered contents paragraph.
function enclosure_contents_phrase(container, contents) {
    return (container.supporter ? "On " : "In ") + the(container) + " you see " + contents + ".";
}
...
lamplighter.setParserLanguage({
    ...
    selfWords: ["me", "myself", "self"],
    connectors: ["and"],
    allWords: ["all", "everything"],
    exceptWords: ["but", "except"],
    // Command separators on one typed line. "and" is deliberately absent: it joins the
    // objects of one command ("take lamp and rope"), it does not start a new one.
    sequenceWords: ["then"],
    // AGAIN/G — replay the last command.
    againWords: ["again", "g"],
    disambiguation: disambiguation_prompt,
    unknownReference: (word) => `I don't know what "${word}" refers to.`,
    // Missing-noun prompt: "What/Who/Which way do you want to <phrase>?" — the interrogative from
    // the empty slot's type, the phrase being what the player typed so far ("take" → "take").
    ...
    // A multiple-object result ("take lamp and rope") is grammatically plural, so it files
    // under the plural object pronoun "them" — the same word a plural collective object uses,
    // unifying the two sources of "them".
    groupAntecedentWords: () => [PRONOUN_PRESETS.they.object],
});
