...
// « (sur la chaise) » / « (dans le placard) » — une clause d'en-tête par objet englobant.
function enclosure_clause(p) {
    if (!p) return "";
    return (p.supporter ? " (sur " : " (dans ") + the(p) + ")";
}

// « Sur la chaise vous voyez … » / « Dans le placard vous voyez … » — un paragraphe de contenus.
function enclosure_contents_phrase(container, contents) {
    return (container.supporter ? "Sur " : "Dans ") + the(container) + " vous voyez " + contents + ".";
}
...
lamplighter.setParserLanguage({
    ...
    // Input-side preposition+article contractions, the mirror of the [au X]/[du X]
    // output sugar above: a grammar literal "à" matches a typed "au" (= à + le),
    // the split-off article joining the following noun phrase, so
    // "accrocher le manteau au crochet" parses against "accrocher … à [x]".
    // Exact-match pass runs first, so a grammar spelling "au"/"du" literally still wins.
