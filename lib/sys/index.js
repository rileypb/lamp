...
function split_commands(line) {
    return lamplighter.splitCommands(line);
}

function command_ran() {
    return lamplighter.commandRan();
}

function input_ended() {
    return lamplighter.inputEnded();
}
...
function set_reach_gate(fn) {
    lamplighter.setReachGate(fn);
}

function set_persuasion_gate(fn) {
    lamplighter.setPersuasionGate(fn);
}
...
function window_sync_one(w) {
    lamplighter.windowSyncOne(w);
}
...
// Freestyle canvas-pane primitives — same thin pass-through shape
// (devdocs/freestyle-windows.md).
function canvas_rect(w, color, x, y, wd, ht) {
    lamplighter.canvasRect(w, color, x, y, wd, ht);
}

function canvas_line(w, color, x1, y1, x2, y2) {
    lamplighter.canvasLine(w, color, x1, y1, x2, y2);
}

function canvas_text(w, color, x, y, size, text) {
    lamplighter.canvasText(w, color, x, y, size, text);
}

function canvas_image(w, img, x, y, wd, ht) {
    lamplighter.canvasImage(w, img, x, y, wd, ht);
}

function canvas_hotspot(w, x, y, wd, ht, command) {
    lamplighter.canvasHotspot(w, x, y, wd, ht, command);
}

function window_kind_available(kind) {
    return lamplighter.windowKindAvailable(kind);
}

// Custom-shell event primitives — same thin pass-through shape
// (devdocs/custom-shells.md).
function shell_send(name, payload) {
    lamplighter.shellSend(name, payload);
}

function shell_available() {
    return lamplighter.shellAvailable();
}
...
// [fit]: shrink-to-fit block on the web shell; ignored elsewhere. See the
// declaration comment in functions.lamp for the single-print contract.
function fit(value) {
    return lamplighter.styled("fit", value);
}
...
function evaluate_scenes() {
    lamplighter.evaluateScenes();
}

function begin_scene(s) {
    lamplighter.beginScene(s);
}

function end_scene(s) {
    lamplighter.endScene(s);
}

function end_all_scenes() {
    lamplighter.endAllScenes();
}
