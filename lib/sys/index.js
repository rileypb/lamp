...
// Integer remainder, non-negative for a positive divisor (Lamp has no `%`). b === 0
// yields 0 rather than NaN, keeping the result an int.
function mod(a, b) {
    const bi = Math.trunc(b);
    if (bi === 0) return 0;
    return ((Math.trunc(a) % bi) + bi) % bi;
}
...
// randomized text. The general randomness primitive; higher-level draws (shuffles,
// dice) build on it in Lamp.
function random(n) {
